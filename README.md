...
- **Code Playground** — run JavaScript and React/JSX code in-browser with live preview and streaming console logs; 96 built-in templates across JS fundamentals (incl. Map/Set, Spread/Rest), interview topics, React basics, **24 JS polyfills** (Array map/filter/reduce/forEach/find/some/every/flat/includes/from/sort/indexOf/reverse/slice/splice/concat, Function bind/call/apply, Promise all/allSettled/race/any, Object.assign + keys/values/entries, String padStart/padEnd, JSON.stringify), coding challenges (find duplicates, custom sorts, anagrams, throttle, EventEmitter, LRU cache, Binary Search, Roman numerals, Linked List, etc.), and React machine coding (Star Rating, Tabs, Accordion, OTP Input, Tic-Tac-Toe, Stopwatch, Calculator, Auto-suggest, Toast, Carousel and more); 33 challenges include a "Show Solution" toggle to reveal the canonical answer; templates browser is a centered 2-pane modal (categories + snippet cards)
